  ...
  do {
   ...
   switch(alloc_mech){
      ...
      case(4): /* Random fit policy*/
			 ...
			 chosenCandidate = random()%(candidateCntr);
			 chosenCandidate++;
			 while (hp != NIL_HOLE && hp->h_base < swap_base && chosenCandidate > 0)
			 {
				if(hp->h_len >= clicks && !(--chosenCandidate)) 
				{
					break;
				}
				prev_ptr = hp;
...
PUBLIC int do_allocmech(void){
   switch(call_nr){
      ...
      case(PM_ALLOC_SET):
		 ...
		 if(alloc_mech == 0)
		 {
			printf("First fit allocation policy is set\n");
		 }
		 else if(alloc_mech == 1)
		 {
			printf("Next fit allocation policy is set\n");
		 }
		 else if(alloc_mech == 2)
		 {
			printf("Worst fit allocation policy is set\n");
		 }
		 else if(alloc_mech == 3)
		 {
			printf("Best fit allocation policy is set\n");
		 }
		 else if(alloc_mech == 4)
		 {
			printf("Random fit allocation policy is set\n");
		 }
		 return OK;
         break;
		default:
         panic(__FILE__,"Bad Signal!!! PANIC MODE", NO_NUM);
   }
}
